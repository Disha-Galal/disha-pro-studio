...
export async function runBrowserChecks() {
  ...
  await test('PDF blank page warning and magic bytes',async()=>{takeNotes();const data=await (await fetch('/qa/sample.pdf')).arrayBuffer();const {text}=await readDocument(new File([data],'mislabeled.txt'),{encoding:'utf-8',ocr:false,update:()=>{}});ok(text.includes('Hello PDF')&&text.includes('Last page'));ok(takeNotes().some(n=>n.includes('1 صفحة')));eq(takeNotes(),[]);});
  await test('Oversized file rejected before parsing',()=>rejects(()=>readDocument(new File([new Uint8Array(20*1024*1024+1)],'large.txt'),{encoding:'utf-8',ocr:false,update:()=>{}}),/20 MB/));
  await test('Image requires explicit OCR',()=>rejects(()=>readDocument(new File(['x'],'photo.png'),{encoding:'utf-8',ocr:false,update:()=>{}}),/OCR/));
  await test('Empty text rejected',()=>rejects(()=>readDocument(new File(['  '],'empty.txt'),{encoding:'utf-8',ocr:false,update:()=>{}}),/لم يتم العثور/));
